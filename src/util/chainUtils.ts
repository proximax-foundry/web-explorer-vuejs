...
export class ChainUtils {
  ...
  static async getChainConfig(
    ...
    return new Promise((resolve, reject) => {
      chainConfigHttp.getChainConfig(chainHeight).subscribe(
        (configString) => {
          const regex = /[^=\n{1}]+=\s*(.*)/g;
          ...
          if (configs?.length) {
            ...
            const chainConfig = <NetworkConfig>{
              ...
              blockGenerationTargetTime:
                networkConfig["blockGenerationTargetTime"],
              ...
              harvestBeneficiaryPercentage:
                ChainUtils.convertConfigNumberToInteger(
              ...
              newCosignersMustApprove:
                networkConfig["newCosignersMustApprove"] === "true"
                  ? true
                  : false,
              ...
              reservedRootNamespaceNames:
                networkConfig["reservedRootNamespaceNames"],
  ...
  static async getNamespacesFromAccount(
    address: Address
  ): Promise<NamespaceInfo[]> {
    const chainRESTCall = new ChainAPICall(
      ChainUtils.buildAPIEndpoint(currentEndPoint.value, connectionPort.value)
    ...
    const namespaceList =
      await chainRESTCall.namespaceAPI.getNamespacesFromAccount(address);
