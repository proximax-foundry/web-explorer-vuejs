...
import type {  TxnExchangeOffer, RestrictionModification, TxnList } from "@/models/transactions/transaction";
...
export class TransactionUtils {
  ...
  static getTransactionTypeNameByEnum(transactionType: TransactionType): string{
    ...
    const foundType  = transactionTypes.find(txnType=> txnType.id == transactionType)
  ...
  static async getRecipient(transferTxn: TransferTransaction, blockHeight: number): Promise<Address>{
    ...
    let recipient: Address = Address.createFromPublicKey( "0".repeat(64) , AppState.networkType)
  ...
  static async extractInnerTransaction(innerTransaction: InnerTransaction, txnGroupType: string): Promise<InnerTxnDetails |  null> {
    ...
    switch (innerTransaction.type) {
      case TransactionType.ADDRESS_ALIAS: {
        ...
        let addressInfo: TxnDetails = {
            type: MsgType.NONE,
            label: "Address",
            value: addressAliasFormat.address?? Address.createFromPublicKey("0".repeat(64),AppState.networkType).plain()
        };
      ...
      case TransactionType.CHAIN_UPGRADE:{
        ...
        let versionInfo: TxnDetails = {
            type: MsgType.NONE,
            label: "New Version",
            value: "v" + parseInt(chainUpgradeFormat.newVersion.substring(0, 4), 16) + "." + parseInt(chainUpgradeFormat.newVersion.toString().substring(4, 8), 16) + "." + parseInt(chainUpgradeFormat.newVersion.toString().substring(8, 12), 16)
        };
      ...
      case TransactionType.MOSAIC_SUPPLY_CHANGE:{
        ...
        if(assetSupplyFormat.supplyDelta == null || assetSupplyFormat.supplyDelta == undefined){
          throw new Error("Service Unavailable")
        }
  ...
  static async extractUnconfirmedTransfer(transferTxn: TransferTransaction): Promise<InnerTransferTransaction> {
    ...
    if(txnDetails.messageType === MessageType.PlainMessage && txnDetails.message){
  ...
  static async extractPartialTransfer(transferTxn: TransferTransaction): Promise<InnerTransferTransaction> {
    ...
    if(namespaceIds.length || allAssetId.length){
        ...
        for(let x= 0; x < sdas.length; ++x){
          ...
          else if( currentAlias.length && AppState.registeredToken.find(rt => rt.fullNamespace === currentAlias[0].name)){
            const findNamespace = AppState.registeredToken.find(rt => rt.fullNamespace === currentAlias[0].name)
            if(findNamespace){
  ...
  static async formatAggregateTransaction(transaction: Transaction, groupType: string): Promise<UnconfirmedAggregateTransaction|ConfirmedAggregateTransaction|PartialAggregateTransaction>{
    ...
    let txn :ConfirmedAggregateTransaction | PartialAggregateTransaction |  UnconfirmedAggregateTransaction
    if(groupType == 'partial'){
      formattedTxn = await TransactionUtils.formatPartialTransaction(transaction);
  ...
  static async formatExchangeTransaction(transaction: Transaction, groupType: string): Promise<ConfirmedExchangeTransaction|UnconfirmedExchangeTransaction|PartialExchangeTransaction>{
    ...
    let formattedTxn:any
    let txn:PartialExchangeTransaction | UnconfirmedExchangeTransaction | ConfirmedExchangeTransaction
    if(groupType == 'partial'){
      formattedTxn = await TransactionUtils.formatPartialTransaction(transaction);
  ...
  static async formatRestrictionTransaction(transaction: Transaction, groupType: string): Promise<ConfirmedRestrictionTransaction|UnconfirmedRestrictionTransaction|PartialRestrictionTransaction>{
    ...
    let formattedTxn:any
    let txn:PartialRestrictionTransaction | UnconfirmedRestrictionTransaction | ConfirmedRestrictionTransaction
    if(groupType == 'partial'){
      formattedTxn = await TransactionUtils.formatPartialTransaction(transaction);
