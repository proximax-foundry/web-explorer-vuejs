        ...
        <div class="md:flex md:justify-between">
          <div class="flex items-center justify-center mb-3 md:mb-0">
            <router-link :to="{ name: 'Home' }"><img src="@/assets/img/sirius-explorer-logo-whitetext.svg"
                class="w-36 h-20 mr-7"></router-link>
          </div>
...
const search = async () => {
  ...
  let searchResult = await searchService.search(
    searchText.value,
    searchFilter.value
  );
  if (searchResult.valid) {
    ...
    switch (searchResult.searchType) {
      ...
      case "Address":
        router.push({
          name: "ViewAccount",
          params: { accountParam: searchResult.param },
        });
        break;
      case "Block":
      ...
      case "Hash":
        router.push({
          ...
          params: { hash: searchResult.param },
        });
        break;
      case "Namespace":
        router.push({
          name: "ViewNamespace",
          params: { namespaceParam: searchResult.param },
        });
        break;
      case "PublicKey":
        router.push({
    ...
    router.push({
      ...
      params: { type: searchResult.searchType, param: searchResult.param },
    });
  }
};
...
let placeHolderString = [
  {
    label: "all",
    placeHolder: "Search by Address / Txn Hash / Namespace / Asset ID",
  },
  ...
  {
    label: "namespaceID",
    placeHolder: "Search by Namespace ID or Namespace Name",
  },
...
const searchPlaceHolder = computed(() => {
  const findPlaceholder = placeHolderString.find(
    (type) => type.label == searchFilter.value
  );
