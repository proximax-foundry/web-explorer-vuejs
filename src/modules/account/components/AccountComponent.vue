      ...
      <div class="flex flex-col justify-center ml-2">
        ...
        <div class="flex items-center">
          <div
            ...
            :copyValue="address"
            copySubject="Address"
            class="text-xs font-semibold mt-1 break-all"
          >
            {{ address }}
          </div>
          <img
        ...
        <div class="text-xxs font-semibold text-gray-400 mt-4 mb-1">
          Public Key
        </div>
        <div class="flex items-center" v-if="publicKey === invalidPublicKey">
          <div id="publicKey" class="text-xs font-semibold mt-1 text-gray-300">
        ...
        <div v-if="linkAccount != invalidPublicKey">
          ...
          <div class="flex items-center">
            <router-link
              :to="{
                name: 'ViewAccount',
                params: { accountParam: linkAccount },
              }"
              id="linked"
              :copyValue="linkAccount"
              copySubject="Linked Account Public Key"
        ...
        <div
          class="text-xxs font-semibold text-gray-400 mt-4 mb-1"
          v-if="namespace.length > 0"
        >
          Linked namespace{{ namespace.length > 1 ? "s" : "" }}
        </div>
        <div class="flex items-center" v-if="namespace.length > 0">
          ...
          >
            ...
            >
            <span
              v-if="namespace.length > 1 && index != namespace.length - 1"
        ...
        <div class="flex gap-2">
          <div
            v-if="multisig > 0"
            class="px-2 py-1 flex mt-4 bg-orange-primary rounded-sm items-center"
          >
            <img
              src="@/modules/account/img/icon-key.svg"
              class="h-3 w-3 mr-1"
              title="This is your default account everytime you login"
            />
            <p
              class="font-semibold text-white text-xxs pt-px cursor-default"
              title="This is a multisig account"
            >
              MULTISIG
            </p>
          </div>
