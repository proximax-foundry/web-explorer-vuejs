...
<div class="border-2 border-t-0">
  <div>
    ...
    <div v-for="(metadata, index) in accountMetadata" :key="index" class="grid grid-cols-5 text-xs md:hidden py-6 px-3 items-center" :class="`${(index != (accountMetadata.length - 1))?'gray-line':''}`">
    ...
    <div v-for="(metadata, index) in accountMetadata" :key="index" class="hidden md:grid md:grid-cols-8 py-6 px-3 items-center text-xs" :class="`${(index != (accountMetadata.length - 1))?'gray-line':''}`">
      ...
      <div class="py-3 px-2 col-span-2 flex items-center">{{metadata.value}}</div>
    </div>
  </div>
</div>
</template>
