<template>
  <div class="border-2 border-t-2">
    <div v-if="accountAssets.length == 0 && isFetching == false"
      class='text-blue-primary text-xs text-center font-semibold py-5'>No asset available</div>
    ...
    <div v-else-if="accountAssets.length > 0 && isFetching == false">
      ...
      <div v-for="(asset, index) in accountAssets" :key="index"
        class="grid grid-cols-5 text-xs md:hidden py-6 px-3 items-center"
        :class="`${(index != (accountAssets.length - 1)) ? 'gray-line' : ''}`">
        <div class="px-2 py-3 col-span-2">
          <router-link :to="{ name: 'ViewAsset', params: { id: asset.id } }"
            class="uppercase text-blue-600 hover:text-blue-primary hover:underline">{{ asset.id }}</router-link>
        </div>
        <div class="py-3 col-span-1">
          <div class="items-center">
            <div class="inline-block mr-2" v-if="asset.name.length != 0">
              <img v-if="displayTokenName(asset.name[0].name).name == 'XPX'" src="@/modules/account/img/proximax-logo.svg"
                class="inline-block h-7 w-7 mr-2 border-2 rounded-3xl">
              <img v-else-if="displayTokenName(asset.name[0].name).name == 'XAR'"
                src="@/modules/account/img/xarcade-logo.svg" class="inline-block h-7 w-7 mr-2 border-2 rounded-3xl">
              <img v-else-if="displayTokenName(asset.name[0].name).name == 'METX'" src="@/modules/account/img/metx-logo.svg"
                class="inline-block h-7 w-7 mr-2 border-2 rounded-3xl">
              <img v-else src="@/modules/transaction/img/proximax-logo-gray.svg" class='inline-block h-6 w-6 mr-2 '>
            </div>
            <div v-for="(namespaceAlias, modifier) in asset.name" :key="modifier"
              class="py-3 col-span-4 flex items-center">
              <!-- <div v-if="namespaceAlias.namespaceId"> -->
              <router-link :to="{ name: 'ViewNamespace', params: { namespaceParam: namespaceAlias.name } }"
                class="text-blue-600 hover:text-blue-primary hover:underline">{{ namespaceAlias.name }}</router-link>
              <span v-if="asset.name.length > 1 && modifier != asset.name.length - 1" class="text-black mr-1 ml-1">{{ "/"
              }}</span>
              <!-- </div> -->
            </div>
          </div>
      ...
      <div v-for="(asset, index) in accountAssets" :key="index"
        class="hidden md:grid md:grid-cols-8 py-6 px-3 items-center text-xs gray-line">
        <router-link :to="{ name: 'ViewAsset', params: { id: asset.id } }"
          class="uppercase text-blue-600 hover:text-blue-primary hover:underline mx-2 my-3 col-span-2">{{ asset.id
          }}</router-link>
        ...
        <div v-else class="py-3 col-span-4 flex items-center ml-2">
          ...
          <div class="inline-block mr-2" v-if="asset.name[0].name && asset.name.length != 0">
            <img v-if="displayTokenName(asset.name[0].name).name == 'XPX'" src="@/modules/account/img/proximax-logo.svg"
              class="inline-block h-7 w-7 mr-2 border-2 rounded-3xl">
            <img v-else-if="displayTokenName(asset.name[0].name).name == 'XAR'" src="@/modules/account/img/xarcade-logo.svg"
              class="inline-block h-7 w-7 mr-2 border-2 rounded-3xl">
            <img v-else-if="displayTokenName(asset.name[0].name).name == 'METX'" src="@/modules/account/img/metx-logo.svg"
              class="inline-block h-7 w-7 mr-2 border-2 rounded-3xl">
            <img v-else src="@/modules/transaction/img/proximax-logo-gray.svg" class='inline-block h-6 w-6 mr-2 '>
          ...
          <div v-for="(namespaceAlias, modifier) in asset.name" :key="modifier" class="py-3 col-span-3 flex items-center">
            <!-- <div v-if="namespaceAlias.namespaceId" > -->
            <router-link :to="{ name: 'ViewNamespace', params: { namespaceParam: namespaceAlias.name } }"
              class="text-blue-600 hover:text-blue-primary hover:underline">{{ displayTokenName(namespaceAlias.name).name
              }}</router-link>
            <!-- </div> -->
            <span v-if="asset.name.length > 1 && modifier != asset.name.length - 1" class="text-black mr-1 ml-1">{{ "/"
            }}</span>
      ...
      <div v-if="isPagination" class="ml-5 mr-5 sm:flex sm:justify-between my-5">
        ...
        <div class="sm:flex sm:items-center text-center sm:text-right">
          <div v-if="enableFirstPage" @click="naviFirst"
            ...
            First</div>
          <div v-else
            class="bg-gray-50 inline-block border border-gray-50 rounded-sm px-2 py-1 text-gray-700 text-xs mx-1">First
          </div>
          <div v-if="enablePreviousPage" @click="naviPrevious"
            class="bg-blue-100 inline-block border border-blue-100 rounded-sm px-2 py-1 text-blue-700 text-xs mx-1 cursor-pointer hover:bg-blue-200 duration-300 transition-all">
            Previous</div>
          <div v-else
            class="bg-gray-50 inline-block border border-gray-50 rounded-sm px-2 py-1 text-gray-700 text-xs mx-1">Previous
          </div>
          <div class="bg-gray-50 inline-block border border-gray-50 rounded-sm px-2 py-1 text-gray-700 text-xs">Page {{
            currentPage }} of {{ totalPages }}</div>
          <div v-if="enableNextPage" @click="naviNext"
            class="bg-blue-100 inline-block border border-blue-100 rounded-sm px-2 py-1 text-blue-700 text-xs mx-1 cursor-pointer hover:bg-blue-200 duration-300 transition-all">
            Next</div>
          <div v-else
            class="bg-gray-50 inline-block border border-gray-50 rounded-sm px-2 py-1 text-gray-700 text-xs mx-1">Next
          </div>
          <div v-if="enableLastPage" @click="naviLast"
            ...
            Last</div>
          <div v-else
            class="bg-gray-50 inline-block border border-gray-50 rounded-sm px-2 py-1 text-gray-700 text-xs mx-1">Last
          </div>
        </div>
      </div>
    </div>
...
const loadAsset = async () => {
  ...
  for(let i =0; i< mosaics_list.length;i++){
    ...
    mosaicIds.push(new Mosaic(new MosaicId(mosaic.id), UInt64.fromUint(mosaic.amount)))
  }
  let assetList = await AccountUtils.formatAccountAsset(mosaicIds , prop.accountPublicKey);
