<template>
  <div class="border-2 border-t-2">
    <div
      v-if="accountAssets.length == 0 && isFetching == false"
    ...
    <div v-else-if="accountAssets.length > 0 && isFetching == false">
      ...
      <div
        v-for="(asset, index) in accountAssets"
        :key="index"
      ...
      >
        ...
        <div class="py-3 col-span-1">
          <div class="items-center">
            <div class="inline-block mr-2" v-if="asset.name.length != 0">
              <img
                v-if="displayTokenName(asset.name[0].name).name == 'XPX'"
                src="@/modules/account/img/proximax-logo.svg"
                class="inline-block h-7 w-7 mr-2 border-2 rounded-3xl"
              />
              <img
                v-else-if="displayTokenName(asset.name[0].name).name == 'XAR'"
                src="@/modules/account/img/xarcade-logo.svg"
                class="inline-block h-7 w-7 mr-2 border-2 rounded-3xl"
              />
              <img
                v-else-if="displayTokenName(asset.name[0].name).name == 'METX'"
            ...
            <div
              v-for="(namespaceAlias, modifier) in asset.name"
              :key="modifier"
            ...
            >
              ...
              <router-link
                :to="{
                  name: 'ViewNamespace',
                  params: { namespaceParam: namespaceAlias.name },
                }"
                class="text-blue-600 hover:text-blue-primary hover:underline"
                >{{ namespaceAlias.name }}</router-link
              >
              <span
                v-if="
                  asset.name.length > 1 && modifier != asset.name.length - 1
                "
        ...
        <div class="py-3 pl-6 flex items-left w-12">
          <div class="inline-block text-xs font-bold">
            {{ splitBalance(asset.balance).left }}
          </div>
          <div
            class="inline-block text-xs font-bold"
            v-if="splitBalance(asset.balance).right != null"
          >
            .
          </div>
          <div class="inline-block text-txs font-bold">
            {{ splitBalance(asset.balance).right }}
          </div>
        </div>
      </div>
      <div
      ...
      >
        <router-link
          :to="{ name: 'ViewAsset', params: { id: asset.id } }"
          class="uppercase text-blue-600 hover:text-blue-primary hover:underline mx-2 my-3 col-span-2"
          >{{ asset.id }}</router-link
        >
        <div
          v-if="asset.name.length == 0"
          class="py-3 col-span-4 flex items-center"
        ></div>
        <div v-else class="py-3 col-span-4 flex items-center ml-2">
          ...
          <div
            ...
            v-if="asset.name[0].name && asset.name.length != 0"
          >
            <img
              v-if="displayTokenName(asset.name[0].name).name == 'XPX'"
              src="@/modules/account/img/proximax-logo.svg"
          ...
          <div
            v-for="(namespaceAlias, modifier) in asset.name"
            :key="modifier"
            class="py-3 col-span-3 flex items-center"
          >
            <!-- <div v-if="namespaceAlias.namespaceId" > -->
            <router-link
              :to="{
              ...
              class="text-blue-600 hover:text-blue-primary hover:underline"
              >{{ displayTokenName(namespaceAlias.name).name }}</router-link
            >
            <!-- </div> -->
            <span
              v-if="asset.name.length > 1 && modifier != asset.name.length - 1"
              class="text-black mr-1 ml-1"
              >{{ "/" }}</span
            >
          </div>
