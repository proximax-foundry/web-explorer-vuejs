<template>
  <div class="border-2 border-t-0 shadow-lg schema">
    <div
      class="p-6 filter overflow-auto transform"
      :class="`${
    ...
    >
      <div class="transform">
        <blocks-tree
          :data="graph"
          :horizontal="viewDirectionSetting"
          :collapsable="collapsable"
          :props="{
            label: 'label',
            numApproveTx: 'numApproveTx',
            numRemoval: 'numRemoval',
            children: 'children',
          }"
        >
          <template #node="{ data }">
            <div class="flex flex-col justify-center p-1.5 h-20 w-72">
              ...
              >
                <div class="text-xs text-left text-blue-500 font-bold">
                  {{ data.name }}
                </div>
                <div class="flex mb-3">
                  <div
                    :id="data.label"
                    :copyValue="prettyAddress(data.label)"
                    copySubject="Address"
                    class="font-bold text-left text-xs mt-0.5"
                  >
                    {{ displayAddress(data.label) }}
                  </div>
                  <img
                    src="@/assets/img/icon-copy.svg"
                    @click="copy(data.label)"
                    class="ml-2 w-4 h-4 cursor-pointer"
                  />
                </div>
                <div class="flex">
                  <div class="mr-2 text-xxs text-gray-500">
                    S: {{ data.numApproveTx }}-of-{{ data.children.length }}
                  </div>
                  <div class="text-xxs text-gray-500">
                    D: {{ data.numRemoval }}-of-{{ data.children.length }}
                  </div>
                  <div class="flex ml-auto">
                    <div
                      v-if="data.children.length > 0"
                      class="ml-auto bg-green-500 rounded-2xl w-7 h-7 flex items-center justify-center"
                      title="Multisig"
                    >
                      <img
                    ...
                    <div
                      v-if="
                        ...
                        findAccountWithAddress(data.label) != undefined
                      "
                      class="w-7 h-7 flex items-center justify-center bg-purple-500 rounded-2xl ml-1.5"
                      title="Owner"
                    >
                      <img
                        src="@/assets/img/icon-bookmark_white.svg"
                        class="h-4 w-4"
                      />
                    </div>
                  </div>
  ...
  <div class="mt-5">
    ...
    <div>
      <input
        ...
        type="radio"
        :value="true"
        v-model="viewDirectionSetting"
        :checked="true"
      />
      <label for="horizontal" class="text-left py-3 text-xs pl-4 cursor-pointer"
        >Horizontal</label
      >
    </div>
    <div>
      <input
        ...
        v-model="viewDirectionSetting"
      />
      <label for="vertical" class="text-left py-3 text-xs pl-4 cursor-pointer"
        >Vertical</label
      >
    </div>
    <div class="mt-3">
      <input
        id="detail1"
        name="view-type-detail"
        type="radio"
        :value="false"
        v-model="viewDirectionDetailSetting"
        :checked="true"
      />
      <label
        for="detail1"
        v-if="viewDirectionSetting"
        class="text-left py-3 text-xs pl-4 cursor-pointer"
        >Right to Left</label
      >
      <label
        for="detail1"
...
const findCosign = (publicKey: string, multisigAccounts: MultisigInfo[]) => {
  const findAcc = multisigAccounts.find(
...
const generateGraph = async (strAddress: string) => {
  ...
  const multisigAccounts = multisigInfos.filter(
    (accounts) =>
      accounts.level >= 0 || accounts.publicKey == props.accountPublicKey
  );
  ...
  graph.value = {
    ...
    children: getChildObject(
      multisigAccounts[0].cosignaturies,
      multisigAccounts
    ),
  };
  isFetch.value = false;
