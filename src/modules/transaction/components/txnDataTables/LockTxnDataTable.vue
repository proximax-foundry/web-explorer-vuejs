  ...
  <div>
    <DataTable :value="transactions" :paginator="false" :rows="Number(pages)" scrollDirection="horizontal"
      ...
      <Column style="width: 200px" v-if="!wideScreen">
        <template #body="{ data }">
          ...
          <div>
            ...
            <div class="flex items-center">
              <div class="font-bold text-xs inline-flex" v-tooltip.right="data.lockHash"><span
                  class="text-ellipsis overflow-hidden w-44">{{ data.lockHash }}</span>...</div>
            </div>
          </div>
        </template>
      </Column>
        ...
        <template #body="{ data }">
          ...
          <div>
            ...
            <div class="uppercase font-bold "
              v-tooltip.left="'<tiptext>Approximately ' + durationTime(data.duration) + ' Day' + (durationTime(data.duration) > 1 ? 's' : '') + '</tiptext>'">
              {{ data.duration ? data.duration + ' Block' + (data.duration > 1 ? 's' : '') : "-" }}</div>
          </div>
          <div>
      ...
      <Column header="Lock Hash" headerStyle="width:40px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
          <span class="text-xs truncate inline-flex text-ellipsis overflow-hidden w-44"
            v-tooltip.bottom="data.lockHash">{{ data.lockHash }}</span>...
        </template>
      </Column>
      <Column header="Duration" headerStyle="width:40px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
          <span class="text-xs font-bold "
            v-tooltip.left="'<tiptext>Approximately ' + durationTime(data.duration) + ' Day' + (durationTime(data.duration) > 1 ? 's' : '') + '</tiptext>'">{{ data.duration
              ? data.duration + ' Block' + (data.duration > 1 ? 's' : '') : "-" }}</span>
        </template>
      </Column>
      <Column header="Refunded" v-if="selectedGroupType === transactionGroupType.CONFIRMED && wideScreen"
