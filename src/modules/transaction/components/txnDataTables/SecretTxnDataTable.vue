  ...
  <div>
    <DataTable
      ...
      <Column field="recipient" header="Recipient" headerStyle="width:110px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{data}">
          <router-link v-tooltip.bottom="Helper.createAddress(data.recipient).pretty()" :to="{ name: 'ViewAccount', params: { accountParam: data.recipient }}" class="hover:text-blue-primary text-blue-600 hover:underline truncate inline-block text-xs">
            {{ Helper.createAddress(data.recipient).pretty() }}
          </router-link>
        </template>
      </Column>
