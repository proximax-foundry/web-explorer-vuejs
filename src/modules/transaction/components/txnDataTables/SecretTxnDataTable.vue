<template>
  <div>
    <DataTable :value="transactions" :paginator="false" :rows="Number(pages)" scrollDirection="horizontal"
      ...
      <Column style="width: 200px" v-if="!wideScreen">
        <template #body="{ data }">
          ...
          <div v-if="data.recipient != '' && data.recipient != null">
            ...
            <router-link v-tooltip.bottom="Helper.createAddress(data.recipient).pretty()"
              :to="{ name: 'ViewAccount', params: { accountParam: data.recipient } }"
              class="hover:text-blue-primary font-bold text-blue-600 hover:underline truncate text-xs inline-flex">
              <span class="text-ellipsis overflow-hidden w-44">{{ Helper.createAddress(data.recipient).pretty()
              }}</span>...
            </router-link>
          </div>
        </template>
      ...
      <Column style="width: 200px" v-if="!wideScreen">
        <template #body="{ data }">
          ...
          <div>
            ...
            <span class="inline-block bg-blue-200 text-blue-700 rounded py-1 px-2 my-1 text-xs font-bold"
      ...
      <Column field="hash" header="TX HASH" headerStyle="width:120px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
      ...
      <Column field="block" header="Block" v-if="selectedGroupType === transactionGroupType.CONFIRMED && wideScreen"
      ...
      <Column header="TX FEE" v-if="selectedGroupType === transactionGroupType.CONFIRMED && wideScreen"
        ...
        <template #body="{ data }">
          <div>
            <img src="@/modules/transaction/img/icon-message.svg"
              v-tooltip.left="'<tiptitle>Secret</tiptitle><tiptext>' + data.secret + '</tiptext>'" class="inline-block">
          </div>
        </template>
      </Column>
      <Column header="Proof" headerStyle="width:40pxwidth:40px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
          <div>
            <img src="@/modules/transaction/img/icon-message.svg" v-if="data.proof"
              v-tooltip.left="'<tiptitle>Proof</tiptitle><tiptext>' + data.proof + '</tiptext>'" class="inline-block">
          </div>
        </template>
      </Column>
      <Column header="SDA" headerStyle="width:30px; text-align:center;width:40px;text-transform:uppercase"
        v-if="wideScreen">
        <template #body="{ data }">
          <div>
            <img src="@/modules/transaction/img/proximax-logo-gray.svg" v-if="data.assetId" class="inline-block w-4/6"
              v-tooltip.left="'<tiptitle>Sirius Digital Asset</tiptitle><tiptext>' + constructSDA(data.assetId, data.amount, data.namespaceName) + '</tiptext>'">
          </div>
        </template>
      </Column>
      <Column header="Info" headerStyle="width:40px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
          ...
          <span class="inline-block bg-blue-200 text-blue-700 rounded py-1 px-2 my-1 mx-1 text-xs font-bold"
            v-if="data.duration">{{ `Duration: ${data.duration} blocks` }}</span>
          <span v-else>-</span>
        </template>
      </Column>
...
const constructSDA = (assetId :string, amount :number, namespaceName :string) => {
  return namespaceName ? amount + ' ' + namespaceName : assetId + ' - ' + amount.toString();
}
