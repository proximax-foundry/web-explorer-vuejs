<template>
  <div>
    <DataTable :value="transactions" :paginator="false" :rows="Number(pages)" scrollDirection="horizontal"
      ...
      <Column style="width: 200px" v-if="!wideScreen">
        <template #body="{ data }">
          <div>
            ...
            <router-link class="uppercase font-bold text-xs block text-blue-600 hover:text-blue-primary hover:underline"
              ...
              <span
                ...
                v-tooltip.right="data.hash">{{ data.hash }}</span>...</router-link>
          </div>
          <div>
          ...
          <div>
            ...
            <div class="flex items-center">
              <router-link :to="{ name: 'ViewAccount', params: { accountParam: data.signerAddress } }"
                v-tooltip.right="Helper.createAddress(data.signerAddress).pretty()"
                class="uppercase font-bold truncate inline-flex text-xs  text-blue-600 hover:text-blue-primary hover:underline w-44"><span
        ...
        <template #body="{ data }">
          ...
          <div>
            ...
            <div class="inline-grid grid-cols-1 gap-2 break-all">
              <div v-bind:key="cosigner" v-tooltip.bottom="'Adding account:<br><br>' + cosigner"
              ...
              <div v-bind:key="cosigner" v-tooltip.bottom="'Removing account:<br><br>' + cosigner"
      ...
      <Column field="hash" header="TX Hash" headerStyle="width:70px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
          <router-link :to="{ name: 'ViewTransaction', params: { hash: data.hash } }"
            ...
            v-tooltip.bottom="data.hash"><span
              class="text-ellipsis overflow-hidden w-44">{{ data.hash }}</span>...</router-link>
        </template>
      </Column>
      <Column field="timestamp" header="Timestamp" headerStyle="width:110px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
      ...
      <Column field="block" header="Block" v-if="wideScreen" headerStyle="width:110px;text-transform:uppercase">
        <template #body="{ data }">
      ...
      <Column header="TX FEE" v-if="wideScreen" headerStyle="width:110px;text-transform:uppercase">
        <template #body="{ data }">
