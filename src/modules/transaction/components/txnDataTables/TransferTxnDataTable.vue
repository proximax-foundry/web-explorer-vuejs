<template>
  <div>
    <DataTable :value="transactions" :paginator="false" :rows="Number(pages)" scrollDirection="horizontal"
      ...
      currentPageReportTemplate="" :globalFilterFields="['recipient', 'sender', 'signerAddress', 'type']">
        ...
        <template #body="{ data }">
          ...
          <div>
            ...
            <div class="flex items-center">
              ...
              <div class="ml-2" v-if="accountAddress">
                <img src="@/modules/transaction/img/icon-txn-out.svg" class="inline-block"
                  v-if="data.sender === Helper.createAddress(accountAddress).plain()">
          ...
          <div>
            ...
            <div class="uppercase font-bold text-xs">
              ...
              <router-link :to="{ name: 'ViewAccount', params: { accountParam: data.recipient } }"
                v-tooltip.right="Helper.createAddress(data.recipient).pretty()" v-else
                class="truncate inline-flex text-xs break-all text-blue-600 hover:text-blue-primary hover:underline w-44"><span
                  class="text-ellipsis overflow-hidden">{{ Helper.createAddress(data.recipient).pretty()
                  }}</span>...</router-link>
            </div>
          </div>
        </template>
        ...
        <template #body="{ data }">
          ...
          <div>
            ...
            <div class="uppercase font-bold text-xs">
              ...
              <router-link :to="{ name: 'ViewAccount', params: { accountParam: data.sender } }" v-else
                v-tooltip.bottom="Helper.createAddress(data.sender).pretty()"
                class="truncate inline-flex w-44 text-xs break-all text-blue-600 hover:text-blue-primary hover:underline"><span
                  class="text-ellipsis overflow-hidden">{{ Helper.createAddress(data.sender).pretty()
                  }}</span>...</router-link>
            </div>
          </div>
          <div>
            <div class="uppercase text-xs text-gray-300 font-bold mb-1 mt-5">Tx Amount</div>
            <div class="text-xs uppercase font-bold">{{ data.amountTransfer ? Helper.toCurrencyFormat(data.amountTransfer,
              currencyDivisibility) : '-' }} <b v-if="data.amountTransfer">{{ nativeTokenName }}</b></div>
          </div>
        </template>
      </Column>
      <Column field="In/Out" header="IN/OUT" headerStyle="width:100px" v-if="wideScreen && accountAddress">
        <template #body="{ data }">
          <div class="ml-2">
            <img src="@/modules/transaction/img/icon-txn-out.svg" class="inline-block"
              v-if="Helper.createAddress(data.sender).plain() === Helper.createAddress(accountAddress).plain()">
            <img src="@/modules/transaction/img/icon-txn-in.svg" class="inline-block" v-else>
          </div>
        </template>
      </Column>
      <Column field="hash" header="TX HASH" headerStyle="width:50px" v-if="wideScreen">
        <template #body="{ data }">
          <router-link :to="{ name: 'ViewTransaction', params: { hash: data.hash } }"
            ...
            v-tooltip.bottom="data.hash"><span class="text-ellipsis overflow-hidden w-20">{{ data.hash }}</span>...
          </router-link>
        </template>
      </Column>
      ...
      <Column field="signer" header="FROM" headerStyle="width:130px" v-if="wideScreen">
        <template #body="{ data }">
          ...
          <router-link :to="{ name: 'ViewAccount', params: { accountParam: data.sender } }" v-else
            v-tooltip.bottom="Helper.createAddress(data.sender).pretty()"
            class="truncate inline-flex text-xs text-blue-600 hover:text-blue-primary hover:underline w-32"><span
              class="text-ellipsis overflow-hidden">{{ Helper.createAddress(data.sender).pretty() }} </span>...
          </router-link>
        </template>
      </Column>
      <Column field="recipient" header="RECIPIENT" headerStyle="width:130px" v-if="wideScreen">
        <template #body="{ data }">
          <span v-if="data.recipient === '' || data.recipient === null"></span>
          <router-link :to="{ name: 'ViewAccount', params: { accountParam: data.recipient } }"
            v-tooltip.bottom="Helper.createAddress(data.recipient).pretty()" v-else
      ...
      <Column header="AMOUNT" headerStyle="width:110px" v-if="wideScreen">
        <template #body="{ data }">
          <div class="text-xs" v-if="data.amountTransfer">{{ Helper.toCurrencyFormat(data.amountTransfer,
            currencyDivisibility) }}</div>
        ...
        <template #body="{ data }">
          <div v-if="data.amountTransfer">
            <span v-if="data.amountTransfer">
              <router-link :to="{ name: 'ViewNamespace', params: { namespaceParam: AppState.nativeToken.fullNamespace } }"
                class="text-blue-600 hover:text-blue-primary hover:underline">{{ nativeTokenName }}</router-link>
            </span>
          </div>
          <div v-if="checkOtherAsset(data.sda)">
            <span v-for="(sdaName, index) in displaySDAs(data.sda)" :key="index">
              <router-link :to="{ name: 'ViewAsset', params: { id: sdaName.name } }"
                class="text-blue-600 hover:text-blue-primary flex hover:underline">{{ sdaName.name }}</router-link>
              {{ data.sda.length }}
            </span>
          </div>
        </template>
      ...
      <Column header="MESSAGE" headerStyle="width:40px" v-if="wideScreen">
        <template #body="{ data }">
          <div class="flex justify-center">
            <img src="@/modules/transaction/img/icon-message.svg"
    ...
    for (const sda of sdas) {
      if (sda.currentAlias && sda.currentAlias.length) {
        if (sda.currentAlias[0].name) {
          sda_rows.push({ amount: sda.amount, name: sda.currentAlias[0].name, divisibility: countDecimals(sda.amount) });
        }
      }
      else {
        sda_rows.push({ amount: sda.amount, name: sda.id, divisibility: countDecimals(sda.amount) });
      }
    }
    return sda_rows;
  }
