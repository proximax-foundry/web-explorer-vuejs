  ...
  <div>
    <DataTable :value="transactions" :paginator="false" :rows="Number(pages)" scrollDirection="horizontal"
      ...
      <Column style="width: 200px" v-if="!wideScreen">
        <template #body="{ data }">
          <div v-if="selectedGroupType === transactionGroupType.CONFIRMED">
            <div class="uppercase text-xs text-gray-300 font-bold mb-1 pt-3">Remote Public Key</div>
            <div class="uppercase font-bold text-xs truncate inline-flex w-44" v-tooltip.bottom="data.remotePublicKey">
              <span class="  text-ellipsis overflow-hidden ">{{ data.remotePublicKey }}</span>...
            </div>
          </div>
          <div>
            <div class="uppercase text-xs text-gray-300 font-bold mb-1 mt-5">Info</div>
            <div v-if="data.action === 'Link'"
      ...
      <Column header="Remote Public Key" headerStyle="width:40px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
          <div class="text-xs inline-flex" v-tooltip.bottom="data.remotePublicKey"> <span
              class="text-ellipsis overflow-hidden w-44">{{ data.remotePublicKey }}</span>...</div>
        </template>
      </Column>
