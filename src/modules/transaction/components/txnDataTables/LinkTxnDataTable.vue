<template>
  <div>
    <DataTable
      ...
      <Column style="width: 200px" v-if="!wideScreen">
        <template #body="{data}">
          <div v-if="selectedGroupType === transactionGroupType.CONFIRMED">
            ...
            <div class="uppercase font-bold text-xs truncate inline-flex w-44" v-tooltip.bottom="data.remotePublicKey"><span class="  text-ellipsis overflow-hidden ">{{data.remotePublicKey}}</span>...</div>
          </div>
          <div>
      ...
      <Column field="timestamp" header="Timestamp" v-if="selectedGroupType === transactionGroupType.CONFIRMED && wideScreen" headerStyle="width:200px;text-transform:uppercase">
        <template #body="{data}">
      ...
      <Column header="Remote Public Key" headerStyle="width:40px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{data}">
          <div class="text-xs inline-flex" v-tooltip.bottom="data.remotePublicKey"> <span class="text-ellipsis overflow-hidden w-44">{{data.remotePublicKey }}</span>...</div>
        </template>
      </Column>
