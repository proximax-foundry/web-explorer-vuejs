  ...
  <div>
    <DataTable :value="transactions" :paginator="false" :rows="Number(pages)" scrollDirection="horizontal"
      ...
      <Column style="width: 200px" v-if="!wideScreen">
        <template #body="{ data }">
          ...
          <div>
            ...
            <div class="inline-block bg-blue-200 text-blue-700 rounded py-1 text-xs font-bold px-1 my-1"
              v-if="data.applyHeightDelta">Apply Height Delta: {{ data.applyHeightDelta }}</div>
            <div class="inline-block bg-blue-200 text-blue-700 rounded py-1 text-xs font-bold px-1 my-1"
              v-if="data.newVersion">New Version: {{ data.newVersion }}</div>
            <div class="inline-block bg-blue-200 text-blue-700 rounded py-1 text-xs font-bold px-1 my-1"
              v-if="data.upgradePeriod">Upgrade Period: {{ data.upgradePeriod }}</div>
          </div>
        </template>
      </Column>
      ...
      <Column header="Info" headerStyle="width:40px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
          <span class="inline-block bg-blue-200 text-blue-700 rounded py-1 text-xs font-bold px-1 my-1 mx-1"
            v-if="data.applyHeightDelta">Apply Height Delta: {{ data.applyHeightDelta }}</span>
          <span class="inline-block bg-blue-200 text-blue-700 rounded py-1 text-xs font-bold px-1 my-1 mx-1"
            v-if="data.newVersion">New Version: {{ data.newVersion }}</span>
          <span class="inline-block bg-blue-200 text-blue-700 rounded py-1 text-xs font-bold px-1 my-1 mx-1"
            v-if="data.upgradePeriod">Upgrade Period: {{ data.upgradePeriod }}</span>
        </template>
      </Column>
      <template #empty>
