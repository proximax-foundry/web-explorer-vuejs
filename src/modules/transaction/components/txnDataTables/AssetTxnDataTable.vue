  ...
  <div>
    <DataTable :value="transactions" :paginator="false" :rows="Number(pages)" scrollDirection="horizontal"
          ...
          <div>
            ...
            <div class="flex items-center">
              <div class="text-xs font-bold">
                <router-link :to="{ name: 'ViewAsset', params: { id: data.assetId } }"
          ...
          <div>
            ...
            <div class="inline-grid grid-cols-1 gap-2 break-all">
              <div class="inline-block bg-blue-100 text-blue-700 font-bold text-xs py-1 px-2 my-1 mr-1 rounded"
                v-if="data.nonce">Nonce: {{ data.nonce }} </div>
              <div class="inline-block bg-blue-100 text-blue-700 font-bold text-xs py-1 px-2 my-1 mr-1 rounded"
                v-if="data.divisibility !== null">Divisibility: {{ data.divisibility }} </div>
              <div
                v-bind:class="['inline-block', 'text-black', 'py-1', 'px-2', 'my-1', 'mr-1', 'text-xs', 'font-bold', 'rounded', (data.transferable) ? 'bg-green-200 text-green-700' : 'bg-red-200 text-red-700']"
                v-if="data.transferable !== null">Transferable</div>
              <div
                v-bind:class="['inline-block', 'text-black', 'py-1', 'px-2', 'my-1', 'mr-1', 'text-xs', 'font-bold', 'rounded', (data.supplyMutable) ? 'bg-green-200 text-green-700' : 'bg-red-200 text-red-700']"
                v-if="data.supplyMutable !== null">Supply Mutable</div>
              <div class="inline-block bg-blue-100 text-blue-700 font-bold text-xs py-1 px-2 my-1 mr-1 rounded"
                v-if="data.duration">Duration: {{ data.duration }}</div>
              <div class="inline-block bg-blue-100 text-blue-700 font-bold text-xs py-1 px-2 my-1 mr-1 rounded"
                v-if="data.supplyDelta !== null">Supply:{{ data.supplyDelta > 0 ? "+" +
                  Helper.toCurrencyFormat(data.supplyDelta, 0) : Helper.toCurrencyFormat(data.supplyDelta, 0) }}</div>
              <div v-if="data.levyAssetId">
                ...
                <div v-tooltip.bottom="data.levyRecipient"
                  ...
                  Levy Recipient: <div class="text-ellipsis overflow-hidden inline-flex truncate w-44">{{
                    data.levyRecipient }}</div>...
                </div>
              </div>
            </div>
          </div>
      ...
      <Column field="hash" header="TX Hash" headerStyle="width:100px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
          <router-link :to="{ name: 'ViewTransaction', params: { hash: data.hash } }"
            ...
            v-tooltip.bottom="data.hash"><span class="text-ellipsis overflow-hidden">{{ data.hash
            }}</span>...</router-link>
        </template>
      </Column>
      <Column field="timestamp" header="Timestamp" headerStyle="width:180px;text-transform:uppercase"
      ...
      <Column header="Asset ID" headerStyle="width:110px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
          <div class="text-xs uppercase"><router-link :to="{ name: 'ViewAsset', params: { id: data.assetId } }"
              class="text-blue-600 uppercase hover:text-blue-primary hover:underline">{{ data.assetId }}{{
                data.namespaceName ? `(${data.namespaceName})` : "" }}</router-link></div>
        </template>
      </Column>
      <Column header="Info" headerStyle="width:40px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
          <span class="inline-block bg-blue-100 text-blue-700 font-bold text-xs py-1 px-2 my-1 mx-1 rounded"
            v-if="data.nonce">Nonce: {{ data.nonce }} </span>
          <span class="inline-block bg-blue-100 text-blue-700 font-bold text-xs py-1 px-2 my-1 mx-1 rounded"
            v-if="data.divisibility !== null">Divisibility: {{ data.divisibility }} </span>
          <span
            v-bind:class="['inline-block', 'text-black', 'py-1', 'px-2', 'my-1', 'mx-1', 'text-xs', 'font-bold', 'rounded', (data.transferable) ? 'bg-green-200 text-green-700' : 'bg-red-200 text-red-700']"
            v-if="data.transferable !== null">Transferable</span>
          <span
            v-bind:class="['inline-block', 'text-black', 'py-1', 'px-2', 'my-1', 'mx-1', 'text-xs', 'font-bold', 'rounded', (data.supplyMutable) ? 'bg-green-200 text-green-700' : 'bg-red-200 text-red-700']"
            v-if="data.supplyMutable !== null">Supply Mutable</span>
          <span class="inline-block bg-blue-100 text-blue-700 font-bold text-xs py-1 px-2 my-1 mx-1 rounded"
            v-if="data.duration">Duration: {{ data.duration }} </span>
          <span class="inline-block bg-blue-100 text-blue-700 font-bold text-xs py-1 px-2 my-1 mx-1 rounded"
            v-if="data.supplyDelta !== null">Supply:{{ data.supplyDelta > 0 ? "+" +
              Helper.toCurrencyFormat(data.supplyDelta, 0) : Helper.toCurrencyFormat(data.supplyDelta, 0) }}</span>
