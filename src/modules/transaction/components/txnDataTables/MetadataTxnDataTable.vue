  ...
  <div>
    <DataTable :value="transactions" :paginator="false" :rows="Number(pages)" scrollDirection="horizontal"
      ...
      <Column style="width: 200px" v-if="!wideScreen">
        <template #body="{ data }">
          ...
          <div>
            ...
            <span class="text-xs font-bold inline-flex" v-if="data.metadataTypeName === 'Account'"
              v-tooltip.bottom="data.targetPublicKey"><span class="text-ellipsis overflow-hidden w-44">
          ...
          <div v-if="selectedGroupType !== transactionGroupType.CONFIRMED">
            ...
            <div><img src="@/modules/transaction/img/icon-message.svg" v-if="data.valueChange"
                v-tooltip.left="'<tiptext>' + constructValueDisplay(data) + '</tiptext>'" class="inline-block"></div>
          </div>
      ...
      <Column header="Target" headerStyle="width:60px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
          <div class="text-xs truncate inline-flex block-all" v-if="data.metadataTypeName === 'Account'"
      ...
      <Column header="Value" v-if="selectedGroupType !== transactionGroupType.CONFIRMED && wideScreen"
        headerStyle="width:40px">
        <template #body="{ data }">
          <div>
            <img src="@/modules/transaction/img/icon-message.svg" v-if="data.valueChange"
              v-tooltip.left="'<tiptext>' + constructValueDisplay(data) + '</tiptext>'" class="inline-block">
          </div>
        </template>
      </Column>
      <template #empty>
