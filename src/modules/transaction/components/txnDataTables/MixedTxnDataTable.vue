        ...
        <template #body="{ data }">
          <div class="ml-2">
            <img src="@/modules/transaction/img/icon-txn-out.svg" class="inline-block"
              v-if="data.sender === Helper.createAddress(accountAddress).plain() && data.type === 'Transfer'">
            <img src="@/modules/transaction/img/icon-txn-in.svg" class="inline-block"
              v-else-if="data.sender != Helper.createAddress(accountAddress).plain() && data.type === 'Transfer'">
          </div>
        </template>
      </Column>
        ...
        <template #body="{ data }">
          <div>
            ...
            <router-link class="uppercase font-bold text-xs block text-blue-600 hover:text-blue-primary hover:underline"
              ...
              <span class="text-xs inline-flex text-ellipsis overflow-hidden hover:underline hover:text-blue-primary w-40"
          ...
          <div>
            ...
            <div class="uppercase font-bold text-xs">
              ...
              <router-link :to="{ name: 'ViewAccount', params: { accountParam: data.recipient } }"
                ...
                class="truncate inline-flex text-xs text-blue-600 hover:text-blue-primary hover:underline w-40"><span
        ...
        <template #body="{ data }">
          ...
          <div>
            ...
            <div class="uppercase font-bold text-xs">
              ...
              <router-link :to="{ name: 'ViewAccount', params: { accountParam: data.signerAddress } }" v-else
                v-tooltip.bottom="Helper.createAddress(data.signerAddress).pretty()"
                class="truncate inline-flex text-xs text-blue-600 hover:text-blue-primary hover:underline w-40"><span
                  class="text-ellipsis overflow-hidden">{{ Helper.createAddress(data.signerAddress).pretty() }}
                </span>...</router-link>
            </div>
          </div>
          <div>
      ...
      <Column field="hash" header="TX HASH" headerStyle="width:90px" v-if="wideScreen">
        <template #body="{ data }">
          <router-link :to="{ name: 'ViewTransaction', params: { hash: data.hash } }"
            ...
            v-tooltip.bottom="data.hash"><span class="text-ellipsis overflow-hidden w-44">{{ data.hash
            }}</span>...</router-link>
        </template>
      </Column>
        ...
        <template #body="{ data }">
          ...
          <div v-if="checkOtherAsset(data.sda)">
            <span v-for="(sdaName, index) in displaySDAs(data.sda)" :key="index">
              <router-link :to="{ name: 'ViewNamespace', params: { namespaceParam: sdaName.name } }"
                class="text-blue-600 hover:text-blue-primary hover:underline">{{ sdaName.name }}</router-link>
              {{ data.sda.length }}
            </span>
          </div>
        </template>
