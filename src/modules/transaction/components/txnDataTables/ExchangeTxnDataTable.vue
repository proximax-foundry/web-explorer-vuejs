    ...
    <DataTable :value="transactions" :paginator="false" :rows="Number(pages)" scrollDirection="horizontal"
          ...
          <div>
            ...
            <div v-if="data.isTakingOffer">
              <span v-bind:key="index" v-for="(exchangeOffer, index) in data.exchangeOffers">
                <span class="inline-block bg-green-200 font-bold text-green-700 text-xs rounded py-2 px-1 my-1"
      ...
      <Column header="Offer" headerStyle="width:40px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{ data }">
          <div v-if="data.isTakingOffer">
            <span v-bind:key="index" v-for="(exchangeOffer, index) in data.exchangeOffers">
              <span class="inline-block bg-green-200 font-bold text-green-700 text-xs rounded py-2 px-1 my-1 mx-1"
                v-if="exchangeOffer.type === 'Sell'">
                {{ displayExchangeSDA(exchangeOffer) }}
              </span>
