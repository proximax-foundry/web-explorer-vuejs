<template>
  <div class="details">
    <div>
      <div>Recipient</div>
      <div class="flex">
        <router-link :to="{ name: 'ViewAccount', params: { accountParam: txnDetail.detail.recipient } }"
          class="mr-2 hover:text-blue-primary hover:underline text-blue-600" id="recipient"
          :copyValue="Helper.createAddress(txnDetail.detail.recipient).pretty()" copySubject="Recipient address">
    ...
    <div v-if="txnDetail.detail.amountTransfer">
      <div>Amount</div>
      <div class="relative">
        <span class="font-bold">{{ transferAmount[0] }}</span>{{ transferAmount[1] > 0 ? '.' : '' }}<span class="text-xxs">{{
          transferAmount[1] }}</span>
    ...
    <div v-if="txnDetail.detail.sda.length > 0">
      ...
      <div class="relative">
        <div v-for="sda, index in txnDetail.detail.sda" :key="index">
          <span class="font-bold">{{ sdaAmount[index][0] }}</span>{{ sdaAmount[index][1] > 0 ? '.' : '' }}<span
            class="text-xxs">{{ sdaAmount[index][1] }}</span>
          <img v-if="sda.currentAlias[0] && sda.currentAlias[0].name == 'xarcade.xar'"
            src="@/modules/account/img/xarcade-logo.svg" class="inline-block h-7 w-7 mr-2 ml-2 border-2 rounded-3xl">
          <img v-else-if="sda.currentAlias[0] && sda.currentAlias[0].name == 'prx.metx'"
            src="@/modules/account/img/metx-logo.svg" class="inline-block h-7 w-7 mr-2 ml-2 border-2 rounded-3xl">
          ...
          <div class="inline-block text-gray-400 text-txs hover:text-gray-700 duration-300 transition-all">
            ...
            <!-- {{ sda.name?' / ':'' }} -->
            <router-link :to="{ name: 'ViewAsset', params: { id: sda.id } }"
              class="hover:text-blue-primary hover:underline">{{ sda.label }}</router-link>
          </div>
        </div>
