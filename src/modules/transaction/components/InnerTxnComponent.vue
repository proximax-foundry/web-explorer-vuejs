  ...
  /*for(let i=0; i < temp.length; ++i){
    ...
    for(let k=0; k < txnProperty.length; ++k){
      if(txnProperty[k].name === ''){
          let currentData = txnProperty[k]
          for(let item in currentData){
            if(currentData[item]){
              for(let j=0; j < currentData[item].length; ++j){
                if(currentData[item][j].name === "From" || currentData[item][j].name === "Public Key"){
                  let index = temp[i].indexOf(txnProperty[k])
                  console.log(temp[i].splice(index, 1))
                  temp[i].push(currentData[item][j])
                }
              }
            }
        }
