<template>
  <div class="mt-3 border border-gray-200 p-3" v-for="(item, index) in allInnerTransactions" :key="index">
    <div class="table_div">
      <div>
        <div>Type</div>
        <div>{{ TransactionUtils.getTransactionTypeName(item.type) }}<span class="text-xxs text-gray-500"> (Version: {{
          item.version }})</span></div>
      </div>
      <div>
        <div>Public Key</div>
        <div class="flex items-center">
          <router-link id="publicKey" :to="{ name: 'ViewAccount', params: { accountParam: item.signer.publicKey } }"
            class="hover:text-blue-primary hover:underline text-blue-600" :copyValue="item.signer.publicKey"
      ...
      <div>
        <div>From</div>
        <div class="flex items-center">
          <router-link id="signerAddress"
            :to="{ name: 'ViewAccount', params: { accountParam: item.signer.address.address } }"
            class="hover:text-blue-primary hover:underline text-blue-600"
            :copyValue="Helper.createAddress(item.signer.address.address).pretty()" copySubject="Address">
    ...
    <div class="table_div" v-if="innerTxnExtractedData[index] != undefined">
      <div v-for="(info, infoListindex) in innerTxnExtractedData[index].infoList" :key="infoListindex">
        ...
        <div v-if="info.label.toLowerCase() == 'effective height'">
          <router-link :to="{ name: 'ViewBlock', params: { blockHeight: info.value } }"
            class="text-xs text-blue-600 hover:text-blue-primary hover:underline">{{ info.value }}</router-link>
        </div>
        <div v-else-if="info.label.toLowerCase() == 'namespace'">
          <div><router-link
              :to="{ name: 'ViewNamespace', params: { namespaceParam: info.short ? info.short : info.value } }"
        ...
        <div v-else-if="info.label.toLowerCase() == 'asset'">
          <router-link :to="{ name: 'ViewAsset', params: { id: info.short ? info.short : info.value } }"
            class="text-blue-600 hover:text-blue-primary hover:underline">
            {{ info.short ? info.short : info.value }}
          </router-link>
        </div>
        <div v-else-if="info.label.toLowerCase() == 'account' || info.label.toLowerCase() == 'recipient'"
          class="flex items-center">
          <router-link id="metadataAccount"
            :to="{ name: 'ViewAccount', params: { accountParam: info.short ? info.short : info.value } }"
            class="text-blue-600 hover:text-blue-primary hover:underline"
            :copyValue="info.short ? info.short : info.value" copySubject="Address">
            {{ info.short ? info.short : info.value }}
          </router-link>
      ...
      <div v-if="innerTxnExtractedData[index].infoGreenList.length > 0">
        ...
        <div>{{ innerTxnExtractedData[index].infoGreenList.map(info => info.short ? info.short : info.value).join(", ") }}
        </div>
      </div>
      <div v-if="innerTxnExtractedData[index].infoRedList.length > 0">
        ...
        <div>{{ innerTxnExtractedData[index].infoRedList.map(info => info.short ? info.short : info.value).join(", ") }}
        </div>
      </div>
      <div
        v-if="innerTxnExtractedData[index].infoInfoList.length > 0 && TransactionUtils.getTransactionTypeName(item.type) == 'SDA Supply Change'">
        <div>Supply Delta</div>
        <div>
          {{ innerTxnExtractedData[index].infoInfoList.map(info => info.label == "Supply Direction" ? info.value :
            '').join("") }}
          {{ innerTxnExtractedData[index].infoInfoList.map(info => info.label == "Supply Delta" ?
            Helper.toCurrencyFormat(info.value, 0) : '').join("") }}
        </div>
      </div>
      <div
        v-if="innerTxnExtractedData[index].infoInfoList.length > 0 && TransactionUtils.getTransactionTypeName(item.type) == 'SDA Definition'">
        <div>Transferable</div>
        <div>{{ innerTxnExtractedData[index].infoInfoList.map(info => info.label == "Transferable" ? info.value :
          '').join("") }}</div>
      </div>
      <div
        v-if="innerTxnExtractedData[index].infoInfoList.length > 0 && TransactionUtils.getTransactionTypeName(item.type) == 'SDA Definition'">
      ...
      <div
        ...
        <div>{{ innerTxnExtractedData[index].infoInfoList.map(info => info.label == "Divisibility" ? info.value :
          '').join("")
        }}</div>
      </div>
        ...
        <div>
          <div v-for="sda, index in innerTxnExtractedData[index].sdas" :key="index">
            ...
            <div v-if="sda.namespace" class="inline-block ml-2">
              <img v-if="sda.namespace == nativeTokenNamespace" src="@/modules/account/img/proximax-logo.svg"
                class="inline-block h-7 w-7 mr-2 border-2 rounded-3xl">
              <img v-else-if="sda.namespace == 'xarcade.xar'" src="@/modules/account/img/xarcade-logo.svg"
                class="inline-block h-7 w-7 mr-2 border-2 rounded-3xl">
              <img v-else-if="sda.namespace == 'prx.metx'" src="@/modules/account/img/metx-logo.svg"
                class="inline-block h-7 w-7 mr-2 border-2 rounded-3xl">
              <img v-else src="@/modules/transaction/img/proximax-logo-gray.svg"
                class="inline-block h-7 w-7 mr-2 border-2 rounded-3xl">
              <router-link :to="{ name: 'ViewAsset', params: { id: sda.assetId } }"
                class="text-blue-600 hover:text-blue-primary hover:underline">{{ sda.label }}</router-link>
            </div>
...
import { TransactionUtils, MsgType, InnerTxnLegendType } from '@/util/transactionUtils';
