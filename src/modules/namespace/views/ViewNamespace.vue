      ...
      <div class="filter shadow-xl border border-gray-50 p-5 mb-5 md:mb-15 md:mr-2">
        ...
        <div class="txn-div">
          ...
          <div class="py-4 text-xs grid grid-cols-5">
            ...
            <div class="uppercase col-span-3"><router-link v-if="namespaceInfo.parentId"
                class="text-blue-600 hover:text-blue-primary hover:underline"
                :to="{ name: 'ViewNamespace', params: { namespaceParam: namespaceInfo.parentId } }">{{
                  namespaceInfo.parentId }}</router-link><span v-else>No parent ID</span></div>
          </div>
        </div>
      </div>
      <div class="filter shadow-xl border border-gray-50 p-5 mb-5 md:mb-15 sm:ml-2">
        ...
        <div class="txn-div">
          <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
            ...
            <div class="break-all uppercase col-span-3"><router-link v-if="namespaceInfo.ownerAddress"
                class="text-blue-600 hover:text-blue-primary hover:underline"
                :to="{ name: 'ViewAccount', params: { accountParam: namespaceInfo.ownerAddress } }">{{
                  namespaceInfo.ownerAddress }}</router-link></div>
          </div>
          <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
            <div class="font-bold col-span-2">Owner Public Key</div>
            <div class="break-all uppercase col-span-3"><router-link v-if="namespaceInfo.ownerPublicKey"
                class="text-blue-600 hover:text-blue-primary hover:underline"
                :to="{ name: 'ViewAccount', params: { accountParam: namespaceInfo.ownerPublicKey } }">{{
                  namespaceInfo.ownerPublicKey }}</router-link></div>
          </div>
          <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
    ...
    <div class="filter shadow-xl border border-gray-50 p-5 mb-15">
      <div class="flex items-center mb-4 border-b border-gray-100 relative">
        <div class="w-32 font-bold text-xs text-center p-2 relative"
          :class="`${(currentComponent == 'details') ? '' : 'cursor-pointer'}`" @click="setCurrentComponent('details')">
          Alias/Levels<div v-if="currentComponent == 'details'"
            class="absolute w-full border-b border-yellow-500 transition-all duration-200" style="bottom: -1px;"></div>
        </div>
        <div class="w-32 font-bold text-xs text-center p-2 relative"
          :class="`${(currentComponent == 'metadata') ? '' : 'cursor-pointer'}`" @click="setCurrentComponent('metadata')">
          Metadata<div v-if="currentComponent == 'metadata'"
            class="absolute w-full border-b border-yellow-500 transition-all duration-200" style="bottom: -1px;"></div>
        </div>
      </div>
      ...
      <div v-if="currentComponent == 'details'" class="mt-10">
        <div v-if="(namespaceInfo.alias || namespaceInfo.levels)" class="px-3">
              ...
              <div class="py-3 grid grid-cols-2">
                <div>
                  <div v-if="namespaceInfo.alias.type == 1">
                    <router-link class="uppercase text-blue-600 hover:text-blue-primary hover:underline"
                      :to="{ name: 'ViewAsset', params: { id: namespaceInfo.alias.id } }">
                  ...
                  <div v-else-if="namespaceInfo.alias.type == 2">
                    <router-link class="uppercase text-blue-600 hover:text-blue-primary hover:underline"
                      :to="{ name: 'ViewAccount', params: { accountParam: namespaceInfo.alias.id } }">
                      {{ namespaceInfo.alias.id }}
                    </router-link>
                  </div>
          ...
          <div v-if="namespaceInfo.levels" class="mt-5">
            ...
            <div v-for="level, index in namespaceInfo.levels" :key="index" class="py-3 text-xs grid grid-cols-2 uppercase"
              :class="`${index < (namespaceInfo.levels.length - 1) ? 'border-b border-gray-100' : ''}`">
              <div v-if="level.type.toUpperCase() === namespaceInfo.name.toUpperCase()">
                {{ level.id }}
              </div>
              <div v-else><router-link v-if="level.id" class="text-blue-600 hover:text-blue-primary hover:underline"
                  :to="{ name: 'ViewNamespace', params: { namespaceParam: level.id } }">{{ level.id }}</router-link></div>
              <div>{{ level.type }}</div>
            </div>
          </div>
        </div>
...
import { NamespaceUtils, type namespaceInfoFormatted } from '@/util/namespaceUtil';
...
const namespaceInfo = ref<namespaceInfoFormatted | null >(null);
