<template>
  <div v-if="isShowInvalid">
    ...
    <div class="p-3 bg-yellow-100 text-yellow-700">
      Namespace ID is not available in {{ networkName }}
    </div>
  </div>
  <div v-else-if="namespaceInfo">
    ...
    <div class="md:grid md:grid-cols-2">
      <div
        class="filter shadow-xl border border-gray-50 p-5 mb-5 md:mb-15 md:mr-2"
      >
        ...
        <div class="txn-div">
          ...
          <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
            ...
            <div class="uppercase col-span-3">
              {{ namespaceInfo.type ? "Root" : "Sub" }}
            </div>
          </div>
          <div class="py-4 text-xs grid grid-cols-5">
            ...
            <div class="uppercase col-span-3">
              <router-link
                v-if="namespaceInfo.parentId"
                class="text-blue-600 hover:text-blue-primary hover:underline"
                :to="{
                  name: 'ViewNamespace',
                  params: { namespaceParam: namespaceInfo.parentId },
                }"
                >{{ namespaceInfo.parentId }}</router-link
              ><span v-else>No parent ID</span>
            </div>
          </div>
        </div>
      </div>
      <div
        class="filter shadow-xl border border-gray-50 p-5 mb-5 md:mb-15 sm:ml-2"
      >
        ...
        <div class="txn-div">
          <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
            ...
            <div class="break-all uppercase col-span-3">
              <router-link
                v-if="namespaceInfo.ownerAddress"
                class="text-blue-600 hover:text-blue-primary hover:underline"
                :to="{
                  name: 'ViewAccount',
                  params: { accountParam: namespaceInfo.ownerAddress },
                }"
                >{{ namespaceInfo.ownerAddress }}</router-link
              >
            </div>
          </div>
          <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
            ...
            <div class="break-all uppercase col-span-3">
              <router-link
                v-if="namespaceInfo.ownerPublicKey"
                class="text-blue-600 hover:text-blue-primary hover:underline"
                :to="{
                  name: 'ViewAccount',
                  params: { accountParam: namespaceInfo.ownerPublicKey },
                }"
                >{{ namespaceInfo.ownerPublicKey }}</router-link
              >
            </div>
          </div>
          <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
    ...
    <div class="filter shadow-xl border border-gray-50 p-5 mb-15">
      <div class="flex items-center mb-4 border-b border-gray-100 relative">
        <div
          ...
          :class="`${currentComponent == 'details' ? '' : 'cursor-pointer'}`"
          @click="setCurrentComponent('details')"
        >
          Alias/Levels
          <div
            v-if="currentComponent == 'details'"
        ...
        <div v-if="namespaceInfo.alias || namespaceInfo.levels" class="px-3">
          <div v-if="namespaceInfo.alias">
            ...
            <div class="py-3 text-xs">
              <div
                class="py-3 grid grid-cols-2 font-bold border-b border-gray-100"
              >
                <div v-if="namespaceInfo.alias.type != 0">
                  {{ namespaceInfo.alias.type == 1 ? "Asset ID" : "Address" }}
                </div>
              ...
              <div class="py-3 grid grid-cols-2">
                <div>
                  <div v-if="namespaceInfo.alias.type == 1">
                    <router-link
                      ...
                      :to="{
                        name: 'ViewAsset',
                        params: { id: namespaceInfo.alias.id },
                      }"
                    >
                  ...
                  <div v-else-if="namespaceInfo.alias.type == 2">
                    <router-link
                      ...
                      :to="{
                        name: 'ViewAccount',
                        params: { accountParam: namespaceInfo.alias.id },
                      }"
                    >
                      {{ namespaceInfo.alias.id }}
                    </router-link>
                ...
                <div v-if="namespaceInfo.alias.type != 0">
                  {{ namespaceInfo.alias.type == 1 ? "Asset" : "Address" }}
                </div>
                <div v-else>-</div>
              </div>
            </div>
          ...
          <div v-if="namespaceInfo.levels" class="mt-5">
            ...
            <div
              class="py-3 text-xs grid grid-cols-2 font-bold border-b border-gray-100"
            >
              <div>Namespace ID</div>
              <div>Name</div>
            </div>
            <div
              v-for="(level, index) in namespaceInfo.levels"
              :key="index"
              class="py-3 text-xs grid grid-cols-2 uppercase"
              :class="`${
                index < namespaceInfo.levels.length - 1
            ...
            >
              ...
              <div v-else>
                <router-link
                  ...
                  :to="{
                    name: 'ViewNamespace',
                    params: { namespaceParam: level.id },
                  }"
...
import {
  NamespaceUtils,
  type namespaceInfoFormatted,
} from "@/util/namespaceUtil";
...
const fetchNamespaceDetail = async () => {
  ...
  let validNamespaceData = NamespaceUtils.isValidNamespaceString(
    props.namespaceParam
  );
