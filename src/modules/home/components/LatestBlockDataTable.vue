<template>
  <div class="border divide-y divide-gray-200">
    <div class="font-bold text-base ml-4 mt-4 mb-4">Latest Blocks</div>
    <div v-if="isFetching">
      <div class="flex justify-center items-center border-gray-400 mt-10 mb-10">
        <div
          class="animate-spin rounded-full h-5 w-5 border-b-2 border-navy-primary"
        ></div>
    ...
    <div v-else>
      ...
      >
        <Column style="width: 100px" v-if="!wideScreen">
          <template #body="{ data }">
            <div class="mb-2 px-4">
              ...
              <div class="uppercase font-bold text-xs inline-flex mt-2">
                <router-link
                  :to="{
                    name: 'ViewBlock',
        ...
        <Column style="width: 180px" v-if="!wideScreen">
          <template #body="{ data }">
            <div>
              <div class="uppercase text-xs text-gray-300 font-bold">
                Validator
              </div>
              <div
                class="uppercase font-bold text-xs text-blue-primary inline-flex mt-2"
              >
                <router-link
                  :to="{
                    name: 'ViewAccount',
              ...
              <div class="text-xs text-gray-500 mb-4 mt-1">
                {{
                  data.numTransactions > 1
                    ? data.numTransactions + " trxs"
                    : data.numTransactions + " trx"
                }}
              </div>
            </div>
          </template>
        ...
        <Column style="width: 130px" v-if="!wideScreen">
          <template #body="{ data }">
            <div>
              ...
              <div class="uppercase font-bold text-xs mt-1 ml-1">
                {{
        ...
        >
          <template #body="{ data }">
            <div>
              ...
              <div class="text-txs text-gray-500 mb-4">
                {{
                  countDuration(
                    data.timestamp.compact() +
                      Deadline.timestampNemesisBlock * 1000
