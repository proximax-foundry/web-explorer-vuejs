        ...
        <div class="filter shadow-xl border border-gray-50 p-5 mb-15 md:mr-2">
          ...
          <div class="txn-div">
            ...
            <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
              ...
              <div class="font-bold uppercase" v-if="assets.namespaceId.length > 0">
                <div v-for='(item, index) in assets.namespaceId' :key="index" class="inline-block ">
                  <router-link :to="{ name: 'ViewNamespace', params: { namespaceParam: item.name } }"
                    class="text-xs font-semibold text-blue-600  col-span-3 hover:text-blue-primary uppercase hover:underline">{{
                      item.name }}
                  </router-link>
                  <span v-if="assets.namespaceId.length > 1 && index != assets.namespaceId.length - 1"
                    class="text-black mr-1 ml-1">{{ "/" }}</span>
            ...
            <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
              ...
              <div>
                {{ Helper.toCurrencyFormat(assets.supply, assets.divisibility) == "NaN" ? "" : Helper.toCurrencyFormat(assets.supply, assets.divisibility) }}
              </div>
            </div>
            <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
          ...
          <div class="txn-div">
            <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
              ...
              <div class="break-all uppercase col-span-3">
                <router-link :to="{ name: 'ViewAccount', params: { accountParam: assets.owner } }"
            ...
            <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
              ...
              <div class="col-span-3">
                <router-link :to="{ name: 'ViewBlock', params: { blockHeight: assets.height } }"
                  class="truncate inline-block break-all text-blue-600 hover:text-blue-primary hover:underline">{{ assets.height }}</router-link>
              </div>
            </div>
            <div class="py-4 text-xs grid grid-cols-5 border-b border-gray-100">
      ...
      <div class="filter shadow-xl border border-gray-50 p-5 mb-15">
        <div class="flex items-center mb-4 border-b border-gray-100 relative">
          <div class="w-32 font-bold text-xs text-center p-2 relative"
            :class="`${(currentComponent == 'rich') ? '' : 'cursor-pointer'}`" @click="setCurrentComponent('rich')">Richlist
            <div v-if="currentComponent == 'rich'"
        ...
        <transition name="slide">
          <RichlistComponent v-if="currentComponent == 'rich'" :transactions="richList" :supply="assets.supply"
