<template>
  <div>
    <p class="text-gray-500 mb-5 text-sm font-bold">Blocks</p>
    <div v-if="isFetching">
      ...
      >
        <Column style="width: 200px" v-if="!wideScreen">
          <template #body="{ data }">
            <div>
              <div
                class="uppercase text-xs text-gray-300 font-bold mb-1 mt-1 break-all"
              >
                Block
              </div>
              <router-link
                ...
                class="text-xs text-blue-600 hover:text-blue-primary font-bold hover:underline"
                >{{ data.height.compact() }}</router-link
              >
            </div>
            <div>
              ...
              <div class="uppercase font-bold text-xs">
                <router-link
                  :to="{
                    name: 'ViewAccount',
                    params: { accountParam: data.signer.publicKey },
                  }"
                  class="truncate text-xs text-blue-600 hover:text-blue-primary hover:underline inline-flex w-40"
        ...
        <Column style="width: 200px" v-if="!wideScreen">
          <template #body="{ data }">
            <div>
              <div class="uppercase text-xs text-gray-300 font-bold mb-1 -mt-7">
                Timestamp
              </div>
              <div class="uppercase font-bold text-xs">
                {{ Helper.formatDeadline(data.timestamp.compact()) }}
              </div>
            </div>
            <div>
              <div class="uppercase text-xs text-gray-300 font-bold mb-1 mt-5">
              ...
              <div class="text-xs uppercase font-bold">
                {{
                  TransactionUtils.convertToExactNativeAmount(
                    data.totalFee.compact()
                  ) +
                  " " +
        ...
        >
          <template #body="{ data }">
            <router-link
              ...
              class="truncate text-xs text-blue-600 hover:text-blue-primary hover:underline"
              >{{ data.signer.publicKey }}</router-link
            >
          </template>
        </Column>
        <Column
...
let pagination = (blocks: BlockInfo[]) => {
  let blockValue = blocks.slice(
    (currentPage.value - 1) * pages.value,
